...
func TestMigrationPush(t *testing.T) {
	t.Run("dry run", func(t *testing.T) {
		...
		conn.Query(migration.LIST_MIGRATION_VERSION).
			Reply("SELECT 0")
		// Run test
		err := Run(context.Background(), true, false, true, true, true, dbConfig, fsys, conn.Intercept)
		// Check error
	...
	t.Run("ignores up to date", func(t *testing.T) {
		...
		conn.Query(migration.LIST_MIGRATION_VERSION).
			Reply("SELECT 0")
		// Run test
		err := Run(context.Background(), false, false, false, false, true, dbConfig, fsys, conn.Intercept)
		// Check error
		assert.NoError(t, err)
	})
	...
	t.Run("throws error on connect failure", func(t *testing.T) {
		...
		fsys := afero.NewMemMapFs()
		// Run test
		err := Run(context.Background(), false, false, false, false, true, pgconn.Config{}, fsys)
		// Check error
...
func TestPushAll(t *testing.T) {
	t.Run("ignores missing roles and seed", func(t *testing.T) {
		...
		helper.MockMigrationHistory(conn).
			...
			Reply("INSERT 0 1")
		// Run test
		err := Run(context.Background(), false, false, true, true, true, dbConfig, fsys, conn.Intercept)
		// Check error
		assert.NoError(t, err)
	})
	...
	t.Run("throws error on roles failure", func(t *testing.T) {
		...
		conn.Query(migration.LIST_MIGRATION_VERSION).
			Reply("SELECT 0")
		// Run test
		err := Run(context.Background(), false, false, true, false, true, dbConfig, fsys, conn.Intercept)
		// Check error
	...
	t.Run("throws error on seed failure", func(t *testing.T) {
		...
		// Run test
		err := Run(context.Background(), false, false, false, true, true, dbConfig, fsys, conn.Intercept)
		// Check error
